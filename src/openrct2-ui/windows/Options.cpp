...
#pragma region Misc tab events
        void MiscMouseUp(WidgetIndex widgetIndex)
        {
            switch (widgetIndex)
            {
                ...
                case WIDX_ALLOW_REPEAT_RIDES:
                    ...
                    invalidate();
                    break;
                case WIDX_NEEDS_INTERRUPT:
                    Config::Get().guestLogic.needsInterrupt ^= 1;
                    Config::Save();
                    invalidate();
                    break;
        ...
        void MiscMouseDown(WidgetIndex widgetIndex)
        {
            ...
            switch (widgetIndex)
            {
                ...
                case WIDX_RIDE_DISTANCE_WEIGHT_DOWN:
                    ...
                    invalidateWidget(WIDX_RIDE_DISTANCE_WEIGHT);
                    break;
                case WIDX_INTENSITY_FLOOR_UP:
                    Config::Get().guestLogic.intensityFloorReduction = std::min<uint8_t>(
                        3, Config::Get().guestLogic.intensityFloorReduction + 1);
                    Config::Save();
                    invalidateWidget(WIDX_INTENSITY_FLOOR);
                    break;
                case WIDX_INTENSITY_FLOOR_DOWN:
                    Config::Get().guestLogic.intensityFloorReduction
                        = (Config::Get().guestLogic.intensityFloorReduction > 0)
                        ? Config::Get().guestLogic.intensityFloorReduction - 1
                        : 0;
                    Config::Save();
                    invalidateWidget(WIDX_INTENSITY_FLOOR);
                    break;
        ...
        void MiscDraw(RenderTarget& rt)
        {
            ...
            // Intensity-floor softening is a plain integer (0-3), so it uses the whole-number formatter
            // rather than the *100 two-decimal one the sliders above use.
            ft = Formatter();
            ft.Add<int32_t>(Config::Get().guestLogic.intensityFloorReduction);
            drawText(
                rt,
