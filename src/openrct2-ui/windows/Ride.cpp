...
#pragma region Operating
        ...
        uint16_t operatingOnPrepareDrawMode(uint16_t startY, const Ride* ride, const RideTypeDescriptor& rtd)
        {
            ...
            // Mode (or, for rides with named animation programs, the "Sequence" selector). Declared
            // here so the operating-tweak switch below can suppress its row for named-program rides.
            const auto* programNames = ride->getRideTypeDescriptor().FlatRideRotation.ProgramNames;
            if (programNames != nullptr)
            ...
            if (programNames != nullptr)
            {
                // Named-program (flat ride) rides pick their sequence via the Mode dropdown above,
