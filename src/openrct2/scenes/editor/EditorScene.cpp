...
bool EditorScene::LoadLandscape(const utf8* path)
{
    ...
    //
    // ForceClose (not just CloseAll): CloseAll() honours each window's canClose(), and the object
    // selection window vetoes closing whenever a required object type is unselected - the normal
    // state on the very first landscape load. The window then survives CloseAll, lives through the
    // park import, and fires its onClose() mid-load, which unloads every object the editor's (now
    // stale) selection didn't include - i.e. the entire freshly loaded park's scenery - leaving
    // naked terrain. Force-closing it first (regardless of valid state) runs that cleanup now,
    // against the pre-load editor state where it is harmless; the park load repopulates what it needs.
    ContextForceCloseWindowByClass(WindowClass::editorObjectSelection);
